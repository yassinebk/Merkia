...
const Notification = (props) => {
    ...
    return (
        <div className={`alert alert-${props.notification.type}  absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 link-fit p-8 shadow-xl blur-xl bg-opacity-40 `}>
  <div className="flex-1 link-fit">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="w-6 h-6 md:w-12 md:h-12 lg:h-16 lg:w-16 mx-2 stroke-current">    
        <Type type={props.notification.type}/>
    </svg> 
                <label className="text-3xl font-sans md:text-5xl lg:text-6xl">{props.notification.message}</label>
  </div>
</div>
