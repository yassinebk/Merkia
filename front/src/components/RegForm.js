...
const RegForm = (props) => {
  ...
  const message = 'Welcome To Merkia !  You are now Registered. Enjoy !'
  ...
  const register = (event) => {
    ...
    props.signUp(newUser, message)
    username.reset()
    email.reset()
    password.reset()
    fullname.reset()

    props.setNotif('Welcome you are now registered !', 'success')
    console.log('here')
    history.push('/')
  }
  ...
  const responseGoogle = (res) => {
    ...
    props.googleAuth(postData, message)
    history.push('/')
  }
        ...
        >
          ...
          <div className="text-lg flex flex-col justify-evenly align-center h-auto">
            <div className="flex flex-row justify-evenly w-full text-xl algin-center">
              <label htmlFor="file" className="text-secondary_100">
                Choose Profile Image
              </label>

              <input
                className="text-lg bg-bg h-24 p-4"
                name="photo"
                type="file"
                filename="photo"
            ...
            <GoogleLogin
              ...
              render={(renderProps) => (
                <Button
                  ...
                  variant="secondary"
                  classes="w-3/6 h-full md:w-1/4 flex flex-row items-center justify-evenly"
                >
